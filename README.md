...
3. 备案完成后打开 `https://avatar.miiiao.cn/`，粘贴链接，选择服务器并填写家族。
...
示例包带有 `SAMPLE` 标识。上 / 下键依次切换六个动作和“制作或更换我的角色”页面；该页面在备案期明确显示“备案中，稍后开放”。OK 键暂停或继续动画。只有一帧的动作自动静态展示。电量可读取时显示，无法读取时自然隐藏。

备案期间，角色制作站只部署到 Vercel 默认预览域名做界面与联调验收，不绑定或跳转 `miiiao.cn`，也不把临时地址写入设备。社区用户可先体验示例角色；正式入口在备案、DNS、HTTPS 与后端部署完成后开放。
