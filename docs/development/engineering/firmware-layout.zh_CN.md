...
`avatar` 偏移是 `partitions.csv`、固件校验器与网页 ESP Web Tools 清单共同遵守的
兼容契约。若未同时更新并发布三者，不得移动或调整该分区。
...
在线制作服务返回的 ESP Web Tools 清单只含一个 part：

```json
{"path":"avatar.pack","offset":3211264}
```

十进制偏移对应 `0x310000`。浏览器只写 `avatar` 分区，不替换应用、分区表、
NVS 或 PHY。只有设备已经安装采用本布局的社区固件时，这条路径才兼容。
